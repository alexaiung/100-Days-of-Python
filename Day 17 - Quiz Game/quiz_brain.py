import random

class QuizBrain:
    def __init__(self, q_list):
        self.question_number = 0
        self.question_list = q_list
        self.score = 0

    def still_has_questions(self):
        return self.question_number < len(self.question_list)

    def next_question(self):
        current_question = self.question_list[self.question_number]
        print('Difficulty -', current_question.difficulty.capitalize())
        print('Category -', current_question.category)
        options = QuizBrain.shuffle_answers(current_question)
        print(f'Q.{self.question_number + 1}: {current_question.question}: ')
        for option in options:
            print('-', option)
        user_answer = input('Your answer: ')
        self.question_number += 1
        self.check_answer(user_answer, current_question.correct_answer)

    @staticmethod
    def shuffle_answers(question):
        options = list()
        options.append(question.correct_answer)
        options.extend(question.incorrect_answers)
        random.shuffle(options)
        return options

    def check_answer(self, user_answer, correct_answer):
        if user_answer.lower() == correct_answer.lower():
            print('You got it right!')
            self.score += 1
        else:
            print("That's wrong.")
        print('The correct answer was:', correct_answer)
        print(f'Your current score is: {self.score}/{self.question_number}')
        print('\n')